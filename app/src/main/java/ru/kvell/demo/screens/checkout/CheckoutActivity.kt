package ru.kvell.demo.screens.checkout
...
import ru.kvell.sdk.api.models.KvellPaymentsTransaction
import ru.kvell.sdk.card.Card
import ru.kvell.demo.R
import ru.kvell.demo.api.PayApi
import ru.kvell.demo.base.BaseActivity
import ru.kvell.demo.databinding.ActivityCheckoutBinding
import ru.kvell.demo.googlepay.PaymentsUtil
import ru.kvell.demo.managers.CartManager
import ru.kvell.demo.support.Constants
import ru.kvell.sdk.ui.dialogs.ThreeDsDialogFragment
...
class CheckoutActivity : BaseActivity(), ThreeDsDialogFragment.ThreeDSDialogListener {
	...
	private fun checkResponse(transaction: KvellPaymentsTransaction?) {
	...
	private fun checkThreeDsResponse(response: KvellPaymentsTransactionResponse) {
		if (response.success == true) {
			showToast("Успешно!")
			finish()
		} else {
			// Показываем результат
			showToast(response.message ?: response.transaction?.cardHolderMessage)
		}
	}

	private fun show3DS(transaction: KvellPaymentsTransaction?) {
		// Открываем 3ds форму
	...
	private fun post3ds(md: String, paRes: String) {
		compositeDisposable.add(
			PayApi.postThreeDs(md.toIntOrNull() ?: 0, paRes)
				.subscribeOn(Schedulers.io())
				.observeOn(AndroidSchedulers.mainThread())
				.doOnSubscribe { showLoading() }
				.doOnEach { hideLoading() }
