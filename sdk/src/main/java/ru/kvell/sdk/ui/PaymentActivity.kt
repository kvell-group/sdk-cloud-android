...
import ru.kvell.sdk.configuration.PaymentConfiguration
import ru.kvell.sdk.dagger2.KvellPaymentsComponent
import ru.kvell.sdk.dagger2.KvellPaymentsModule
import ru.kvell.sdk.dagger2.KvellPaymentsNetModule
import ru.kvell.sdk.dagger2.DaggerKvellPaymentsComponent
import ru.kvell.sdk.databinding.ActivityKvellPaymentBinding
import ru.kvell.sdk.models.PayParams
...
internal class PaymentActivity: FragmentActivity(), BasePaymentBottomSheetFragment.IPaymentFragment,
	...
	override fun finish() {
		super.finish()
		overridePendingTransition(R.anim.kvell_fade_in, R.anim.kvell_fade_out)
	}

	internal val component: KvellPaymentsComponent by lazy {
		DaggerKvellPaymentsComponent
			.builder()
			.kvellPaymentsModule(KvellPaymentsModule())
			.kvellPaymentsNetModule(KvellPaymentsNetModule(paymentConfiguration!!.publicId, paymentConfiguration!!.apiSecret, paymentConfiguration!!.apiUrl))
			.build()
	}
	...
	private lateinit var binding: ActivityKvellPaymentBinding
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		binding = ActivityKvellPaymentBinding.inflate(layoutInflater)
	...
	override fun onPaymentFinished(transactionId: Int) {
		setResult(Activity.RESULT_OK, Intent().apply {
			putExtra(KvellPaymentsSDK.IntentKeys.TransactionId.name, transactionId)
			putExtra(KvellPaymentsSDK.IntentKeys.TransactionStatus.name, KvellPaymentsSDK.TransactionStatus.Succeeded)
		})
	}
	...
	override fun onPaymentFailed(transactionId: Int, reasonCode: Int?) {
		setResult(Activity.RESULT_OK, Intent().apply {
			putExtra(KvellPaymentsSDK.IntentKeys.TransactionId.name, transactionId)
			putExtra(KvellPaymentsSDK.IntentKeys.TransactionStatus.name, KvellPaymentsSDK.TransactionStatus.Failed)
			reasonCode?.let { putExtra(KvellPaymentsSDK.IntentKeys.TransactionReasonCode.name, it) }
		})
	}
